...
- **No edit-time recall, no post-compact reminder.** The `PreToolUse` hook that
  surfaces a file's learnings the moment you edit it and the `SessionStart`
  (post-compaction) lore reminder are wired by the **Claude Code** plugin only;
  the Codex installer registers `UserPromptSubmit` (recall) + `Stop` (capture).
  Prompt-time recall, the capture overlap check, and freshness are identical.
